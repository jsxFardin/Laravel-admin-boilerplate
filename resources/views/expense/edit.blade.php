                            ...
                            <div class="row">
                                <div class="form-group col-6">
                                    ...
                                    <select class="form-control @error('expense_type_id') is-invalid @enderror" 
                                        ...
                                        @foreach ($expenseType as $item)
                                            <option value="{{$item->id}}" {{ $item->id == $expense->expense_type_id ? 'selected' : '' }}>{{$item->name}}</option>
                                        @endforeach
                                    </select>
                                </div>
                                <div class="col-md-6 col-xs-12">
                                    <div class="form-group">
                                        ...
                                        <textarea class="form-control  @error('description') is-invalid @enderror" name="description" rows="3">{{ $expense->description }}</textarea>
                                        @error('description')
                            ...
                            <div class="row hide-row">
                                <div class="form-group col-6">
                                    ...
                                    <input type="text" class="form-control @error('travel_form') is-invalid @enderror" 
                                        ...
                                        value="{{ $expense->destination_from }}">
                            ...
                            <div class="row hide-amount-row d-none">
                                <div class="form-group col-6">
                                    ...
                                    <input type="text" class="form-control @error('amount') is-invalid @enderror" 
                                        name="amount" placeholder="Enter amount" id="amount"  value="{{ $expense->amount }}">
                                </div>
                            </div>
                                    ...
                                    <div class="col-12 col-sm-4">
                                        <div class="info-box bg-light">
                                            <div class="info-box-content">
                                                ...
                                                <h4 class="info-box-number text-center text-bold mb-0 value" > {{ $expense->amount ?? 0}}</h4>
                                            </div>
                                        </div>
                                    </div>
                                </div>
    
                                <input type="text" name="destination_id" id="destination_id" value="{{ $expense->destination_id ?? ''}}" class="d-none">
                            </div>
                        </div>
    ...
    <script>
        ...
        function expenseType() {
            ...
            if (expense_type_ == 4) { // FOR OTHERS OPTIONS
                ...
                showCalculatedDiv()
                $('#amount').attr('value', $('#calculated_value').find('.value').text())
            }else {
                $('.hide-row').removeClass('d-none')
                $('.hide-amount-row').addClass('d-block')
                hideCalculatedDiv()
